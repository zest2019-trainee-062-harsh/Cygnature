                            ...
                            >
                                {
                                    ...
                                    this.state.fulldata["annotationShapes"].map((data, index) =>{
                                        if(this.state.fulldata["annotationShapes"][index]["p"] == currentPage){
                                            ...
                                            //console.warn(xCoordinate+" "+yCoordinate)
                                            var _value = {
                                                x: xCoordinate,
                                                y: yCoordinate
                                            }
                                            this.Animatedvalue.addListener((value)=> _value = value)
                                            this.Animatedvalue.setValue({x: _value.x, y: _value.y})
                                            this.PanResponder = PanResponder.create({
                                            onStartShouldSetPanResponder: (evt, gestureState) => false,
                                            onMoveShouldSetPanResponder: (evt, gestureState) => false,
                                            onPanResponderGrant: (e, gestureState) => {
                                            ...
                                            var annotationHeight = (this.state.fulldata["annotationShapes"][index]["h"]/ (height/2))*100;
                                            var annotationWidth = (this.state.fulldata["annotationShapes"][index]["w"]/ (width/1.4))*100
                                            return(
                                                <Animated.View
